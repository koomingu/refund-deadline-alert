    ...
    <div className={`bg-white dark:bg-slate-900 rounded-2xl shadow-sm shadow-gray-900/5 overflow-hidden border transition-all ${isCanceled ? 'opacity-75' : ''} ${isBeingEdited ? 'border-amber-400 ring-2 ring-amber-300' : 'border-gray-200 dark:border-slate-700'}`}>
      <div className="p-4 pb-3">
        ...
        <div className="flex justify-between items-start mb-2">
          <div className="flex items-center gap-2 flex-wrap">
            ...
            <span className="font-bold text-gray-800 dark:text-slate-100">{res.examName}</span>
          ...
          <div className="flex gap-1.5 items-center shrink-0 ml-2">
            ...
            {isScheduled && (
              <button onClick={() => onToggleAlarm(res.id)} title={hasAlarm ? '알림 해제' : '알림 설정'}
        ...
        <div className="flex items-end justify-between">
          <div>
            <div className="text-2xl font-extrabold text-gray-900 dark:text-slate-100 tracking-tight">
            ...
            <div className="text-gray-500 dark:text-slate-400 text-sm mt-0.5">
              ...
              {res.price > 0 && <span className="ml-2 text-indigo-500 dark:text-indigo-400 font-semibold">{fmtMoney(res.price)}</span>}
            </div>
        ...
        {isScheduled && curFeeFmt && !nextTier && (
          <div className={`mt-3 px-4 py-3 rounded-xl flex items-center justify-between ${
            ...
            <span className={`text-xl font-extrabold ${curFeeFmt.isFree ? 'text-green-600 dark:text-green-400' : 'text-red-600 dark:text-red-400'}`}>
              {curFeeFmt.main}
              {curFeeFmt.sub && <span className="text-sm font-normal ml-1 text-gray-400 dark:text-slate-500">({curFeeFmt.sub})</span>}
            </span>
          </div>
        )}
      ...
      {res.isExpanded && !isCanceled && (
        <div className="p-4 bg-gray-50/80 dark:bg-slate-800/50 border-t border-gray-100 dark:border-slate-700">
          {exam?.note && (
            <div className="mb-3 text-xs text-amber-700 dark:text-amber-300 bg-amber-50 dark:bg-amber-900/20 border border-amber-200 dark:border-amber-800 rounded px-2.5 py-2">
              ⚠️ {exam.note}
            </div>
          )}
          <div className="relative border-l-2 border-indigo-200 dark:border-indigo-800 ml-3 space-y-5">
            {timeline.map((tier, index) => {
              ...
              const triggerDisplay = tier.triggerTime
                ...
                : null;
              const dotColor = tier.fee === '무료' ? 'bg-green-400' : tier.fee === '불가' ? 'bg-gray-500 dark:bg-slate-500' : 'bg-indigo-500';

              return (
                <div key={index} className="relative pl-6">
                  <div className={`absolute -left-[9px] top-1 w-4 h-4 rounded-full border-2 border-white dark:border-slate-800 shadow-sm ${isTimePast ? 'bg-gray-300 dark:bg-slate-600' : dotColor}`} />
                  <div className={isTimePast ? 'opacity-40' : ''}>
                    {triggerDisplay && (
                      <div className="text-xs text-gray-400 dark:text-slate-500 font-medium mb-0.5">{triggerDisplay}부터</div>
                    )}
                    <div className="text-xs text-gray-500 dark:text-slate-400 mb-0.5">{tier.label}</div>
                    <div className="flex items-baseline gap-1.5">
                      <span className={`text-lg font-extrabold ${feeFmt.isFree ? 'text-green-600 dark:text-green-400' : tier.fee === '불가' ? 'text-gray-800 dark:text-slate-300' : 'text-red-600 dark:text-red-400'}`}>
                        {feeFmt.main}
                      </span>
